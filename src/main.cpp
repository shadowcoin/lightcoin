...
static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // LightCoin: 7 days
static const int64 nTargetSpacing = 300; // LightCoin: 5 minute blocks
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "High Court Ends Overall Limit on Political Donations, from the wsj.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396537264; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0;

        if (fTestNet)
        {
            block.nTime    = 1396537264;
            block.nNonce   = 0;
        }
